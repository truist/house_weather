...
FORMATTED="$(echo "$DATA" | jq '.data[0].sensors | map({ key: (.comp), value: (.value)}) | from_entries')"
REQ_URL="$(echo "$FORMATTED" | jq -j '"http://weather.rainskit.com/submit?source=awair&temperature=\(.temp)&humidity=\(.humid)&co2=\(.co2)&voc=\(.voc)&pm25=\(.pm25)"')"
